...
func (store *Store) UpdateAutoStopPolicy(ctx context.Context, ownerID string, policy domain.AutoStopPolicy, candidate domain.Operation) (domain.Operation, error) {
	...
	if strings.TrimSpace(ownerID) == "" || ownerID != strings.TrimSpace(ownerID) ||
		...
		return domain.Operation{}, errors.New("update Auto-stop Policy: canonical owner and matching Policy and Operation are required")
	}
	tx, err := store.pool.Begin(ctx)
	if err != nil {
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: begin transaction: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	queries := store.queries.WithTx(tx)
	if _, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock(hashtextextended('auto-stop-policy' || chr(31) || $1 || chr(31) || $2, 0))`, ownerID, operation.IdempotencyKey); err != nil {
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: lock idempotency key: %w", err)
	}
	...
	if err == nil {
		...
		if err := tx.Commit(ctx); err != nil {
			return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: commit replay: %w", err)
		}
		return restored, nil
	}
	if !errors.Is(err, pgx.ErrNoRows) {
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: read idempotency key: %w", err)
	}
	...
	if err := tx.QueryRow(ctx, `
		...
		return domain.Operation{}, ErrReferenceNotOwned
	} else if err != nil {
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: lock owned Environment: %w", err)
	}
	...
	if err := tx.QueryRow(ctx, `SELECT EXISTS (
		...
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: inspect active Operation: %w", err)
	}
	if active {
	...
	if _, err := tx.Exec(ctx, `
		...
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: persist Policy: %w", err)
	}
	if _, err := tx.Exec(ctx, `
		...
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: record Operation: %w", err)
	}
	if err := tx.Commit(ctx); err != nil {
		return domain.Operation{}, fmt.Errorf("update Auto-stop Policy: commit: %w", err)
	}
